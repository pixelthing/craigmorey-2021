...
const cookieRun = () => {
  ...
  const init = () => {
    if (window.location.search.includes('?craig')) {
      console.warn('LOOKS LIKE YOU ARE CRAIG!!!');
      localStorage.setItem('functional.areYouCraig', 'yep');
    } else if (window.location.search.includes('?notcraig')) {
      console.log('you\'re no longer craig...');
      localStorage.removeItem('functional.areYouCraig');
    }
    if (!window.dataLayer) {
      window.dataLayer = [];
    }
