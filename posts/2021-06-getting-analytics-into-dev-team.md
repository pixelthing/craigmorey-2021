...
In my experience, good implementation documentation is never worth dropping when time is tight - it helps you focus better on what you want, helps implentation happen, and can then be tweaked to show the resulting anayltics details (eg GA events) that analysts can then use to understand what happened and why.
...
- In fact, where some interesting actions take place in the site that we don't intend to initially report on, add them in, but be prepared to lose them. This isn't for some evil bargaining reason, but more future-proofing - we all know instances of minor UX in websites suddenly becoming an unexpected priority for management, and having existing hooks ready to measure without hacks or panic are often a godsend.
...
```
- Be flexible about swapping between asking for dataLayer pushes and markup changes. dataLayer pushes are essential for state changes and complex states with lots of information, but they need devs to often create new detailed methods where they never existed before. If you're looking for something simpler, like notification of a click with a small amount of data, asking devs to add a data attribute to their HTML component is often a much quicker task for devs to complete. You could even create generic markup-hooks that mean the dev team's workload is simpler, and eliminates yours [see article here]. For instance:
```js
...
Try to make it clear what you're compromising on, so that everyone feels some pressure to even up the balance and assist you. And explain why you want each point of data, it often overlaps with what the dev team also want to know, and helps contextualize how much effort it's worth.

## Conclusion

If a lot of the above sounds like compromising, you'd be right. If it sounds a lot like business school politics - you _might_ be right (although I'd just call it keeping a good relationship). And it doesn't need to feel cynical, if we're talking about wanting to get the job done _or_ having respect for your co-workers, there's no reason those two beliefs can't be held simultaneously.
